...
exports.execute = async (client, message, args) => {
      ...
      let Name = args[1] 
      let Age = args[2]
      if (!member || !Age || !Name) return message.channel.send({ embeds: [embed.setDescription(`\`[WARN] Örnek kullanım: .Kayıt @chaos\ ID \``)] }).then(msg => {setTimeout(() => msg.delete(), 5000);});
      ...
      message.channel.send({ embeds: [embed.setDescription(`\`${member.displayName}\` Başariyla Yeni İsmi Değiştirildi`)]}).then(msg => {setTimeout(() => msg.delete(), 5000);});
}
exports.conf = {
  command: "isim",
  description: "",
